...
bulk API により、一度の API コールでたくさんの index/delete 処理を実行できます。
これにより、インデキシングの速度を大幅に向上できます。
...
公式にサポートされたいくつかのクライアントでは、ひとつのインデックスから
もうひとつのインデックスまで、bulk リクエストとドキュメントの再インデキシングの
手助けするためのヘルパー(Bulk, Scroll)が提供されています。
...
REST API のエンドポイントは、`/_bulk` とし、次のような JSON 構造を期待します。
